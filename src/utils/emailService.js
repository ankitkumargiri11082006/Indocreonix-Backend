...
export function buildOfferLetterIssuedEmail({ fullName, opportunityTitle, portalUrl }) {
  const subject = `Offer letter issued for ${opportunityTitle}`
  const preview = `Hi ${fullName}, your offer letter has been generated and is awaiting admin approval.`

  const body = `
  ${header('Offer Letter Issued')}
  <div class="tag-strip">
    <span class="tag" style="background:#eff6ff;color:#1d4ed8;border-color:#3b82f622">Offer Letter</span>
    <span class="tag" style="background:#fef3c7;color:#92400e;border-color:#f59e0b22">Awaiting Approval</span>
  </div>
  <div class="body">
    <p class="greeting">Hello ${fullName},</p>
    <p class="intro">Your offer letter for <strong>${opportunityTitle}</strong> has been generated by our HR team.</p>
    <p class="intro">You can view and download the document from your candidate portal as soon as it is approved by admin.</p>
    <div class="btn-wrap">
      <a href="${portalUrl}" class="cta-btn">Open Candidate Portal &rarr;</a>
    </div>
    <p class="intro" style="margin-top:20px">Regards,<br/><strong style="color:${COLOR.btnBg}">Team ${BRAND}</strong></p>
  </div>`

  return { subject, html: shell(preview, body) }
}

export async function sendOfferLetterIssuedEmail(to, data) {
  const { subject, html } = buildOfferLetterIssuedEmail(data)
  return sendMail({
    from:    `"${BRAND} Careers" <${getSenderAddress('careers')}>`,
    to,
    subject,
...
export function buildCertificateIssuedEmail({ fullName, portalUrl }) {
  const subject = `Certificate issued for your completed onboarding`
  const preview = `Hi ${fullName}, your certificate has been issued and is waiting for admin approval.`

  const body = `
  ${header('Certificate Issued')}
  <div class="tag-strip">
    <span class="tag" style="background:#ecfdf5;color:#166534;border-color:#10b98122">Certificate</span>
    <span class="tag" style="background:#fef3c7;color:#92400e;border-color:#f59e0b22">Awaiting Approval</span>
  </div>
  <div class="body">
    <p class="greeting">Hello ${fullName},</p>
    <p class="intro">Your certificate has been generated successfully by our team.</p>
    <p class="intro">As soon as admin approval is completed, you can download it from your candidate portal.</p>
    <div class="btn-wrap">
      <a href="${portalUrl}" class="cta-btn">Open Candidate Portal &rarr;</a>
    </div>
    <p class="intro" style="margin-top:20px">Regards,<br/><strong style="color:${COLOR.btnBg}">Team ${BRAND}</strong></p>
...
export async function sendCertificateIssuedEmail(to, data) {
  const { subject, html } = buildCertificateIssuedEmail(data)
  return sendMail({
    from:    `"${BRAND} Careers" <${getSenderAddress('careers')}>`,
    to,
    subject,
