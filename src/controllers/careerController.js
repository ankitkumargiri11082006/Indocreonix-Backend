...
async function resolveCloudinaryCvDelivery(
  publicId,
  preferredResourceType = "image",
) {
  if (!publicId) return "";

  const resourceTypes =
    preferredResourceType === "raw" ? ["raw", "image"] : ["image", "raw"];
    ...
    try {
      ...
      if (resource?.secure_url) {
        const hasAnonymousAccess = Array.isArray(resource.access_control)
          ? resource.access_control.some(
              (entry) => entry?.access_type === "anonymous",
            )
          : false;
...
export const updateOpportunity = asyncHandler(async (req, res) => {
  const item = await Opportunity.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true,
  });
...
export const submitApplication = asyncHandler(async (req, res) => {
  ...
  const verifiedEmail = String(portalUser.email || "")
    .toLowerCase()
    .trim();
  if (!verifiedEmail) {
  ...
  if (!hasAcceptedConsent) {
    throw new ApiError(
      400,
      "You must accept Indocreonix Terms and Privacy Policy",
    );
  }
  ...
  const resolvedCvUrl =
    uploaded.secure_url ||
    getCloudinaryCvUrl(uploaded.public_id, uploaded.resource_type);
  ...
  const opportunityTitle = opportunityId
    ? (await Opportunity.findById(opportunityId).select("title").lean())
        ?.title || roleType
    : roleType;
  ...
  res
    .status(201)
    .json({ message: "Application submitted successfully", itemId: item._id });
});
...
export const exportApplicationsCsv = asyncHandler(async (req, res) => {
  ...
  await createAuditLog(
    req,
    "EXPORT_CAREER_APPLICATIONS_CSV",
    "CareerApplication",
    "",
    {
      count: items.length,
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename=career-applications-${date}.csv`,
  );
